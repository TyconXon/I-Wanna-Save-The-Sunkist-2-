...
if (argument0=="step") {
    if (ladder) {
    ...
    } else if (hang || hanging) {
        sprite_index=sprPlayerSlidingPG
        image_speed=0.5
    } else if (!onGround) {
        if (vspeed*vflip<-0.05) {
            sprite_index=sprPlayerJumpPG
            image_speed=0
            image_index+=0.5
        ...
        if (vspeed*vflip>0.05) {
            sprite_index=sprPlayerFallPG
            image_speed=0.5
        }
    } else if (input_h!=0) {
            sprite_index=sprPlayerRunningPG
            image_speed=mmf_animspeed(70,80)
    } else {
       sprite_index=sprPlayerIdlePG
       image_speed=0.2
    }
...
if (argument0=="draw") {
    ...
    if (bow) {
        ...
        if ((drawspr==sprPlayerIdlePG || drawspr==sprPlayerIdleOld) && floor(drawframe)==3) dy+=vflip //bobbing
