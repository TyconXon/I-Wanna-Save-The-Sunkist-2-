...
if(global.teleSickness > 0) global.teleSickness-=1
with(Portal){
    if(global.teleSickness > 0) {image_alpha = 0.25;}else{image_alpha = 1}
...
if (!frozen) {
    ...
    if (instance_place(x,y,LowSpeedField)) {
        ...
        maxSpeed = 6
    }else if(instance_place(x,y,AirStop)){
        maxspeed = 0
    }
...
if (!frozen) {
    ...
    if (onGround) {
        if (place_free(x,y+vflip+vspeed) && !instances_place(x,y+4*vflip+vspeed,Platform,PlatformGizmo) && !coyoteTime) {
            onGround=false
        }
    }
    ...
    if (!cutscene) {
        ...
        if(key_down(vi_pressed) && theendisnigh){
            vspeed+=5
            walljumpboost=0
        }
        if (key_die()) {
...
//conveyor blocks
conveyor=instance_place(x,y+4*vflip,Conveyor) if (conveyor) hspeed+=conveyor.spd
if(instance_place(x,y+4*vflip,ConveyerStick)) hspeed = 0
...
#define Collision_PlatformGizmo
/*"/*'/**//* YYD ACTION
lib_id=1
action_id=603
applies_to=self
...
if (!dead) {
    ...
    //draw celeste bow
    if (bow && settings("celeste") && djump>=maxjumps) draw_sprite_ext(sprBow,3,floor(bowx),floor(bowy+abs(lengthdir_y(2,sprite_angle))*vflip+(vflip==-1)),facing,vflip,drawangle,image_blend,image_alpha)
